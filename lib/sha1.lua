...
local include_fields=configuration.sha1_fields
for _, v in ipairs(include_fields) do
	include_fields[v]=true
end
...
function hmac_sha1_message_string(text)
	local m={}
	local in_sha1
--print ("*******************************\n", text, '*******************************\n')
	for attrib, value in string.gmatch(text, "([%d%w%-_]+)%=(%C-)\n") do
--		print ("****", attrib, value)
		if include_fields[attrib] then
			m[#m+1]=attrib..'='..value
		end
		if attrib=='sha1' and value~='@SHA1@' then
--			print ("************ in_sha", value)
			in_sha1=value
		end
	end
...
--print ("===============================\n", s, '===============================\n')
	return hmac_sha1_string(sha1key, s), in_sha1
end

function hmac_sha1_message_table(data)
	local m={}
	local in_sha1
...
--return the singature calculated from the active fields in the message, followed by the content of 
--the sha1 field in the message (if any)
function hmac_sha1_message(message)
	if type(message)=='string' then
		return hmac_sha1_message_string(message)
	end
	if type(message)=='table' then
		return hmac_sha1_message_table(message)
	end
end

function hmac_sha1_message_verify(text)
	local signature, in_signature = hmac_sha1_message(text)
--print ('#####################',signature, in_signature)
	if not in_signature or in_signature=='n/a' then
		return 'missing'
	end
